...
%     p_old  = FirnSetup3('dz',dz(rr));
    p = FirnSetup4('dz',dz(rr));
...
%   Out_old = FCM9(p_old);
    ...
    RunTime = toc
    Out.Age(Out.Age<1e-20) = 0;    % corrext for a small numerical error which yields non zero age at the surface (~1e-30)
    ...
    if abs(dz(rr) - dz_ref) < 1e-10    % only plot in one iteration
        ...
        xlabel(ax1,'nondimensional depth, $\zeta$','FontSize',12); ylabel(ax1,'nondimensional variable','FontSize',12)
    end
    ...
    if abs(dz(rr) - dz_ref) < 1e-10    % only plot in one iteration
        ...
        lgd = legend(h2,'$\phi$','$\sigma$','$w$','$r^2$','$A$','location','southeast','FontSize',12);
    end
...
%         sentenceAboutReachingSteadyState = sprintf(format,temp.time2ss,temp.time2ss*temp.scales.t0/(365*24*60*60))  
    ...
    if abs(dz(rr) - dz_ref) < 1e-10     % only plot in one iteration
        ...
        xlabel(ax2,'nondimensional depth, $\zeta$','FontSize',12); ylabel(ax2,'mismatch between ODE and full-model results','FontSize',12)
        box on
...
axes('Position',[0.7372    0.6908    0.1574    0.19])
...
%% 6. print figure
% print('-dpng','F2_full_ode_comparisons_v6.png')
