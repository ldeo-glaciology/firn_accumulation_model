...
%D2 = three_point_centered_uni_D2(xi_p(1), xi_p(end), N);

%%% Solve the system
time = [0 10];
%options = odeset('RelTol',1e-9,'AbsTol', 1e-12);
% myOutputFcn= @(Time,Vars) plot(Time;
...
%%% Compute gradients.
%    DPHI1 = D1*phi;
%    DPHI1 = [0;(phi(2:end,1) - phi(1:end-1,1))]./p.dxi_p;
%    DT1 = D1*T;
%    DT2 = D2*T;
    DR2_1 = D1*r2;
    DA_1 = D1*A;
...
%%% Change in porosity.
%    dphidt = -w.*DPHI1 - (1/Ar)*sigma.^n .*(1 - phi).*phi.^m .*exp(lambda_c*T)./r2;
%    dphidt = zeros(N,1);
    dphidt = D1*((1 - phi).*w);
